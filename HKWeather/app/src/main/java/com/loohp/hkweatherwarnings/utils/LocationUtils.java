...
public class LocationUtils {
    ...
    public static boolean checkLocationPermission(TileService service) {
        return ActivityCompat.checkSelfPermission(service, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(service, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED;
    }
    ...
    public static CompletableFuture<LocationResult> getGPSLocation(TileService tileService) {
        if (!checkLocationPermission(tileService)) {
            return CompletableFuture.completedFuture(null);
        }
        CompletableFuture<LocationResult> future = new CompletableFuture<>();
        FusedLocationProviderClient client = LocationServices.getFusedLocationProviderClient(tileService);
        ForkJoinPool.commonPool().execute(() -> {
            client.getLocationAvailability().addOnCompleteListener(task -> {
                if (task.isSuccessful() && task.getResult().isLocationAvailable()) {
                    ...
                    LocationManager locationManager = (LocationManager) tileService.getSystemService(Context.LOCATION_SERVICE);
